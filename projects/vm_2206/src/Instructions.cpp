...
// BRANCH 指令：无条件跳转
void BranchInstruction::execute(VMContext& context, int operand) {
    context.instructionCounter = operand;  // 直接设置 PC 为目标地址
}

std::string BranchInstruction::getName() const {
    return "BRANCH";
}

// BRANCHNEG 指令：累加器为负时跳转
void BranchNegInstruction::execute(VMContext& context, int operand) {
...
std::string BranchNegInstruction::getName() const {
    return "BRANCHNEG";
}

// BRANCHZERO 指令：累加器为零时跳转
void BranchZeroInstruction::execute(VMContext& context, int operand) {
    if (context.accumulator == 0) {
        context.instructionCounter = operand;  // 跳转
    } else {
        context.instructionCounter++;  // 继续执行下一条
...
std::string BranchZeroInstruction::getName() const {
    return "BRANCHZERO";
}
